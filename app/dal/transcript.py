...
def create_transcript(db: Session, lesson_id: str, transcription: list[dict]) -> Transcript:
...
def create_feedback(db: Session, transcript_id: int, user_id: int, role: str, strengths: str, improvements: str) -> Feedback:
    feedback = Feedback(transcript_id=transcript_id, user_id=user_id, role=role, strengths=strengths, improvements=improvements)
    db.add(feedback)
    db.commit()
    db.refresh(feedback)
    return feedback
