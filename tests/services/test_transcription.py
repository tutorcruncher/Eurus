...
async def test_handle_webhook_success(
    ...
    with patch.object(
        ...
        transcript = await transcription_service.get_transcript_by_id(lesson_id, db_session)
...
async def test_get_transcript_not_found(transcription_service, db_session):
    with pytest.raises(HTTPException) as exc_info:
        await transcription_service.get_transcript_by_id('non-existent-lesson', db_session)
